...
exports.preLearningBlueprintDetails = async (request, callback) => {
  try {
    ...
    const topicNames = await new Promise((resolve, reject) => {
      ...
      averages.forEach(({ concept_id, topic_id, topic_title, average_marks }) => {
        const conceptData = conceptMap.get(concept_id) || { totalScore: 0, count: 0, topic_id, topic_title };
      ...
      const conceptAverages = [...conceptMap].map(([concept_id, { totalScore, count, topic_id, topic_title }]) => ({
        concept_id,
        topic_id,
        topic_title, // Include topic_title in the concept data
        average_score: totalScore / count,
        number_of_questions: count,
      }));

      console.log("Step 2");
      console.log(conceptAverages);
