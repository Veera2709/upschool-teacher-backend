...
exports.getTopicsBasedonChapters = async function (request, callback) {
  ...
  if(Array.isArray(request.data.chapter_array)){
    if(request.data.chapter_array.length > 0){

      chapterRepository.fetchChaptersIDandChapterTopicID({chapter_array: request.data.chapter_array}, async function (chapter_err, chapter_response) {
        if (chapter_err) { 
            console.log(chapter_err);
            callback(chapter_err, chapter_response);
        } else {
          if(chapter_response.Items.length > 0){
            ...
            topicRepository.fetchTopicIDDisplayTitleData({topic_array: topic_array}, async function (topic_err, topic_response) {
              if (topic_err) { 
                  console.log(topic_err);
                  callback(topic_err, topic_response);
              } else {
                callback(topic_err, topic_response.Items); 
              }
            }) 
          }else{
            callback(200, chapter_response.Items); 
          }
        }
      }) 
    }else{
      callback(200, request.data.chapter_array); 
    }
  }else{
